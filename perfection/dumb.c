...
void drawStuff() {
   ...
   for(n = 0; n < renderCount; ++n) {
      ...
      glBegin(GL_QUADS);   
      unsigned int index_tangent = glGetAttribLocation(shaders[n], (const char*)"tangent");
      unsigned int index_bitangent = glGetAttribLocation(shaders[n], (const char*)"bitangent");
      ...
      for(i = 0; i < object->faceCount; ++i) {
         for(j = 0; j < 4; ++j) {
            ...
            glVertexAttrib3fv(1, (GLfloat*)&object->tangents[vi - 1]);
            glVertexAttrib3fv(3, (GLfloat*)&object->bitangents[vi - 1]);
...
//   loadResources("ground", 2);
//   loadResources("background", 4);
